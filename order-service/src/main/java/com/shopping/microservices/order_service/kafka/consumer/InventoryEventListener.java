        ...
        try {
            ...
            switch (eventType) {
                case INVENTORY_RESERVED -> {
                    orderService.updateOrderProgress(event.getOrderId(), OrderProgress.INVENTORY_RESERVED, null, event.getCorrelationId());
                    log.info("Order {} inventory reserved successfully", event.getOrderId());
                }
                case INVENTORY_INSUFFICIENT -> {
                    orderService.cancelOrder(event.getOrderId(), "Insufficient inventory: " + event.getReason(), event.getCorrelationId());
                    log.warn("Order {} cancelled due to insufficient inventory", event.getOrderId());
                }
                case INVENTORY_CONFIRMED -> {
                    orderService.updateOrderProgress(event.getOrderId(), OrderProgress.READY_TO_SHIP, null, event.getCorrelationId());
